...
1. `<run-dir>/proposal.md` through a proposal writer, reviewer, and explicit
   user approval.
2. `<run-dir>/high-level-design.md` through a design writer, reviewer, and
   explicit user approval.
3. `<run-dir>/spec.md` through `spec-writer`, then the original
   Superteam-style planning and executing chain with runtime guards.
...
project folder without overwriting each other's proposal, design, plan, or task
state. Legacy root files such as `proposal.md` or `working/plan/` are reported
as legacy state and are not treated as the active run.
...
Create <run-dir>/proposal.md first.
Then create <run-dir>/high-level-design.md.
After <run-dir>/proposal.md is reviewed, stop and wait for my explicit approval
before design.
After <run-dir>/high-level-design.md is reviewed, stop and wait for my explicit
